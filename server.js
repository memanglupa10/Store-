...
async function handleRequest(req, res) {
  ...
  if ((pathname === '/api/webhooks/payment' || pathname === '/api/webhook/qris' || pathname === '/api/webhook/payment' || pathname === '/api/webhook/mayar' || pathname === '/api/webhook/midtrans') && method === 'POST') {
    ...
    if (!order) {
      // Fallback 1: Match latest UNPAID order if Midtrans Simulator generated an internal transaction ID (e.g. A1202...)
      order = db.orders.find(o => o.payment_status === 'UNPAID');
    }

    if (!order) {
      // Fallback 2: Reconstruct order record for serverless instance resilience
      order = {
        id: targetOrderId,
  ...
  // =========================================================
  let reqUrl = pathname;
  if (!reqUrl.includes('.') || reqUrl === '/' || reqUrl === '/login' || reqUrl === '/admin' || reqUrl === '/katalog') {
    reqUrl = '/index.html';
  }
